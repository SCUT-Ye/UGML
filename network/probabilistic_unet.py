...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers,num_classes, padding=True, posterior=False):
        ...
        if posterior:
            ...
            self.input_channels += num_classes # changed by ccoco Originial: 1
            # Brain tumor mask : 4
            # Liver tumor mask: 3 class
...
class AxisAlignedConvGaussian(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, latent_dim, initializers, num_classes, posterior=False):
        ...
        if self.posterior:
            ...
            self.name = 'Prior'
        self.encoder = Encoder(self.input_channels, self.num_filters, self.no_convs_per_block, initializers,self.num_classes, posterior=self.posterior)
    ...
    def forward(self, input, segm=None):
        ...
        # add by coco for 3D PU  # change by coco
...
class Fcomb(nn.Module):
    ...
    def forward(self, feature_map, z):
        ...
        if self.use_tile:
            ...
            # add byb coco for PU3D  # change by coco
...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=14, num_filters=[32,64,128,256], latent_dim=6, no_convs_fcomb=4, beta=10.0):
        super(ProbabilisticUnet, self).__init__()
        self.input_channels = input_channels
        self.num_classes = num_classes
        self.num_filters = num_filters
        self.latent_dim = latent_dim
        self.no_convs_per_block = 3 # Orignial: 3
        self.no_convs_fcomb = no_convs_fcomb
        ...
        self.prior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block, self.latent_dim,  self.initializers,self.num_classes).to(device)
        self.posterior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block, self.latent_dim, self.initializers,self.num_classes, posterior=True).to(device)
    ...
    def sample(self, img,testing=False): # change by coco
        """
        ...
        if testing == False:
            ...
            self.forward(img,training=False) # change by coco
    ...
    def sample_m(self, img,m,testing=False): # change by coco
        """
        Sample a segmentation by reconstructing from a prior sample
        and combining this with UNet features
        """
        if testing == False:
            ...
            res = torch.ones(img_num, m, self.num_classes, h, w,t).cuda()
            self.forward(img,training=False) # change by coco
            #You can choose whether you mean a sample or the mean here. For the GED it is important to take a sample.
            #z_prior = self.prior_latent_space.base_dist.loc
            # z_prior = self.prior_latent_space.sample()
    ...
    def elbo(self, segm, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        return -(self.mean_reconstruction_loss + self.beta * self.kl) # change by coco
