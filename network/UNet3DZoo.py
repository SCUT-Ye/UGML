...
class AttUnet(nn.Module):
    def __init__(self, in_channels=1, base_channels=32, num_classes=14):
        ...
        #deepsuper
        self.gt_conv4 = nn.Sequential(nn.Conv3d(256, 1, 1))
        self.gt_conv3 = nn.Sequential(nn.Conv3d(128, 1, 1))
        self.gt_conv2 = nn.Sequential(nn.Conv3d(64, 1, 1))
        self.outconv = nn.Conv3d(num_classes+3, num_classes, 1)
    ...
    def forward(self, x):
        ...
        gt_4 = self.gt_conv4(x4_4)
        gt_3 = self.gt_conv3(y4)
        gt_2 = self.gt_conv2(y3)
        ...
        out = self.outconv(torch.cat((m4, m3, m2, y), 1))
        # out = y+m2+m3+m4
        return out
...
class Unet(nn.Module):
    def __init__(self, in_channels=1, base_channels=32, num_classes=14):
        ...
        self.DeUpCat5 = DeUp_Cat(in_channels=base_channels * 16, out_channels=base_channels * 8)
        self.DeBlock5 = DeBlock(in_channels=base_channels*8)
    ...
    def forward(self, x):
        ...
        y5 = self.DeUpCat5(x5_2, x4_1)
        y5 = self.DeBlock5(y5)

        y4 = self.DeUpCat4(y5, x3_1)
        y4 = self.DeBlock4(y4)

        y3 = self.DeUpCat3(y4, x2_1)  
        y3 = self.DeBlock3(y3)

        y2 = self.DeUpCat2(y3, x1_1)  
        y2 = self.DeBlock2(y2)
        y = self.endconv(y2)
...
class Unetdrop(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.endconv(y2)
        return y
class unAttUnet_v2(nn.Module):
    def __init__(self, in_channels=1, base_channels=32, num_classes=14):
        super(unAttUnet_v2, self).__init__()
        ...
        self.endconv = nn.Conv3d(base_channels, num_classes, kernel_size=1)
        #deepsuper
        self.gt_conv4 = nn.Sequential(nn.Conv3d(256, num_classes, 1))
        self.gt_conv3 = nn.Sequential(nn.Conv3d(128, num_classes, 1))
        self.gt_conv2 = nn.Sequential(nn.Conv3d(64, num_classes, 1))
        self.gt_conv1 = nn.Sequential(nn.Conv3d(32, num_classes, 1))

    def forward(self, x):
        x = self.InitConv(x)      # (1, 32, 128, 128, 128)

        x1_1 = self.EnBlock1(x)
        x1_2 = self.EnDown1(x1_1)  

        x2_1 = self.EnBlock2_1(x1_2) # (1, 64, 64, 64, 64)
        x2_1 = self.EnBlock2_2(x2_1)
        x2_2 = self.EnDown2(x2_1)

        x3_1 = self.EnBlock3_1(x2_2) # (1, 128, 32, 32, 32)
        x3_1 = self.EnBlock3_2(x3_1)
        x3_2 = self.EnDown3(x3_1)  

        x4_1 = self.EnBlock4_1(x3_2) # (1, 256, 16, 16, 16)
        x4_2 = self.EnBlock4_2(x4_1)
        x4_2 = self.EnDown4(x4_2)

        x5_1 = self.EnBlock5_1(x4_2) # (1, 512, 8, 8, 8)
        x5_2 = self.EnBlock5_2(x5_1)  

        y5 = self.DeUp5(x5_2) # (1, 256, 8, 8, 8)
        x4_1 = self.Att5(g=y5,x=x4_1)
        y5= self.DeUpCat5(x4_1, y5)
        y5 = self.DeBlock5(y5)
        ...
        y2 = self.DeUp2(y3)
        x1_1 = self.Att2(g=y2, x=x1_1)
        y2 = self.DeUpCat2(y2, x1_1)
        y2 = self.DeBlock2(y2)
        y = self.endconv(y2)

        return y
class unAttUnet(nn.Module):
    def __init__(self, in_channels=1, base_channels=32, num_classes=14):
        super(unAttUnet, self).__init__()
        ...
        self.DeUp5 = DeUp(in_channels=base_channels*16, out_channels=base_channels*8)
        self.DeUpCat5 = De_Cat(in_channels=base_channels * 16, out_channels=base_channels * 8)
        self.DeBlock5 = DeBlock(in_channels=base_channels*8)
        ...
        self.endconv = nn.Conv3d(base_channels, num_classes, kernel_size=1)
        #deepsuper
        self.gt_conv5 = nn.Sequential(nn.Conv3d(512, 1, 1))
        self.gt_conv4 = nn.Sequential(nn.Conv3d(256, 1, 1))
        self.gt_conv3 = nn.Sequential(nn.Conv3d(128, 1, 1))
        self.gt_conv2 = nn.Sequential(nn.Conv3d(64, 1, 1))
        self.outconv = nn.Conv3d(num_classes+4, num_classes, 1)
    ...
    def forward(self, x):
        ...
        gt_5 = self.gt_conv5(x5_2)
        gt_4 = self.gt_conv4(y5)
        gt_3 = self.gt_conv3(y4)
        gt_2 = self.gt_conv2(y3)
        ...
        m3 = F.interpolate(gt_3, scale_factor=4, mode='trilinear', align_corners=True)
        m2 = F.interpolate(gt_2, scale_factor=2, mode='trilinear', align_corners=True)
        out = self.outconv(torch.cat((m5, m4, m3, m2, y), 1))
        # out = y+m2+m3+m4+m5
        return out
